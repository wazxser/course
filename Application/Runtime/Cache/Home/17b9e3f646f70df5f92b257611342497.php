                ...
                <div style="margin: 60px 30px 30px 30px">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="panel-title"><i class="fa fa-money fa-fw"></i>学生信息</h3>
                        </div>
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-bordered table-hover table-striped" style="width: 100%">
                                    <form action="selectinfo" method="post">
                                        <input type="text" style="width:30%" name="key" class="form-control" placeholder="学号，课程编号">
                                    </form>
                                    <br />
                                    <thead>
                                    <tr>
                                    ...
                                    <?php if(is_array($select)): $i = 0; $__LIST__ = $select;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$select): $mod = ($i % 2 );++$i;?><tr>
                                            <td><?php echo ($select["sid"]); ?></td>
                                            <td><?php echo ($select["cid"]); ?></td>
                                            <!--<td><?php echo ($select["cname"]); ?></td>-->
                                            <td><?php echo ($select["year"]); ?></td>
                                            <td><?php echo ($select["mark"]); ?>分</td>
                                            <td><a href="<?php echo U('Home/Teacher/seldelete');?>?id=<?php echo ($select["id"]); ?>">删除</a></td>
                                            <td><a href="<?php echo U('Home/Teacher/selectModify');?>?id=<?php echo ($select["id"]); ?>">修改</a></td>
                                        </tr><?php endforeach; endif; else: echo "" ;endif; ?>

                                    </tbody>
                                </table>
                                <a href="selectAdd.html"><button type="button" class="btn btn-lg btn-default">Add</button></a>
                            </div>
                        </div>
                    </div>
                </div>
